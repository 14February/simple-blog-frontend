...
const loadBlogs = async (page = 0) => {
    ...
    let res = await axios.post("/blog/article/page", {
        pageIndex: pageInfo.page,
        pageSize: pageInfo.pageSize,
        content: pageInfo.keyword,
        categoryId: pageInfo.categoryId

    })
    console.log(res);
    // ?keyword=${pageInfo.keyword}&pageIndex=${pageInfo.page}&pageSize=${pageInfo.pageSize}&categoryId=${pageInfo.categoryId}`)
    let temp_rows = res.data.data.dataList;
    // 处理获取的文章列表数据
...
const loadCategorys = async () => {
    let res = await axios.get("/blog/category/query")
